...
def OtraceEIC(M, H, p, r, iter_num, plot = True):
    ...
    for iter in range(iter_num):
        for i in range(m):
            ...
            MH = Hi*Mi
            Hid = np.diag(Hi)
            X[i] = np.dot(fractional_matrix_power((Hid+ r*D),-1), MH.T).T
        U,S,V = la.svd(X)
        s = np.zeros(shape = (n,1))
        t = S
        for i in range(len(t)):
            s[i] = t[i]
        D = p/2*fractional_matrix_power((np.dot(X.T, X) + np.dot(st, np.eye(n))),(p/2-1))
        obj.append(np.trace(np.dot((X*H-M).T, X*H-M))+r*sum(np.power((s*s+st), p/2).T[0]))
        print("iter%d  obj:%f" %(iter, (np.trace(np.dot((X*H-M).T, X*H-M))+r*sum(np.power((s+st), p/2).T[0]))))
    if plot:
        plt.figure(1)
